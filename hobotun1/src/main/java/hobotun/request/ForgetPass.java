...
public class ForgetPass implements Serializable {
	...
	public void onSendMail() {
		...
		if (userTbl.isEmpty()) {
			...
			userTbl = userDao.getUserByMailBlock(params);

			if (!userTbl.isEmpty()) {
				Misc.setMessageElement(ID_MSG_FOR_EMAIL_ELEMENT, FacesMessage.SEVERITY_ERROR, MSG_ERROR_EMAIL_BLOCK);
			} else {
	...
	private static final String MSG_ERROR_EMAIL_BLOCK = "Пользовтаель с таком e-mail заблокирован за нарушения!";
