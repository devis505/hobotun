...
public class SelectAllModelOrderByDate extends MappingSqlQuery<ModelTbl> {
	...
	private static final String SQL_SELECT_MODEL_BY_USER_ID = 
			  "SELECT m.*, IFNULL((select sum(rm.vl_rating) from hb_rating_modele rm where rm.id_model = m.idModel), 0) rating, \n"
			...
			+ "	      IF(m.idImg5 is not null, (select p.vlParam from hb_params p where p.idParam = 24), 0 ) price_calc \n"
	        + "  FROM hb_model m\n"
